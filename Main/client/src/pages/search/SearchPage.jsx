...
import { Search, User, Rocket, Users, Rss, Store, AlertCircle, RefreshCw } from 'lucide-react';
...
export const SearchPage = () => {
  ...
  // Execute search whenever URL parameters change
  useEffect(() => {
    executeSearch();
    ...
    <div className="bg-[#F7F5EF] min-h-screen p-4 sm:p-6 font-ui text-[#0E1A2B]">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="p-4 bg-white border border-[#5B6472]/20 rounded-[8px] shadow-[0_2px_8px_rgba(14,26,43,0.03)]">
          <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3 items-end">
            ...
            <Button
              ...
              className="bg-[#0F6E5C] hover:bg-[#0F6E5C]/90 text-white rounded-[4px] px-6 h-10 text-xs font-bold w-full sm:w-auto shadow-none"
            >
              Search
            </Button>
          </form>
        ...
        <div className="space-y-6">
          ...
          {!isLoading && !error && queryParam && !hasAnyResults && (
            <EmptyState
              icon={Search}
              title="No results found"
              description={`We couldn't find any results matching "${queryParam}" in this category.`}
          ...
          {!isLoading && !error && !queryParam && (
            <EmptyState
              ...
              description="Type key terms into the search bar above to query TrustNet."
              className="bg-white border border-[#5B6472]/20 rounded-[8px]"
            />
          )}
