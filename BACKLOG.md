...
- [ ] **No full verification state-transition matrix.** This phase added exactly one guard (a *fresh* decision requires `verificationStatus === "pending"`) and idempotency for *repeating* the same decision. An admin can still reject an already-*approved* account (treated as a deliberate, retained capability - new information can surface after approval) or move directly from `resubmission_requested` to `approved`/`rejected` without a fresh submission in between. Whether any of these cross-state transitions should be restricted is a Phase 16C question, alongside the unified `accountStatus` design.
